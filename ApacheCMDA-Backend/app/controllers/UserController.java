...
public class UserController extends Controller {
	...
	public Result userLogin() {
		...
		if (email.contains("@")) {
			loginAPI = new EmailLoginController(userRepository);
		} else {
			loginAPI = new UsernameLoginController(userRepository);
		}

		LoginController loginController = new LoginController(loginAPI);
    ...
    public Result suggestGroupName(String email, String name){
        GroupUsers groupUsers = new GroupUsers();
        User user = userRepository.findByEmail(email);
        user.suggestGroupName(groupUsers, name);
        JsonObject jsonObject = new JsonObject();
        jsonObject.addProperty("groupName", groupUsers.getGroupName());
        return ok(new Gson().toJson(jsonObject));
    }
