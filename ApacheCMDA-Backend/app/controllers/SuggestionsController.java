...
@Named
@Singleton
public class SuggestionsController extends Template {
    ...
    @Override
    public Result initialize(JsonNode jsonNode) {
        if (jsonNode == null) {
        ...
        long wfID = jsonNode.path("wfID").asLong();
        Workflow workflow = workflowRepository.findById(wfID);
        if(workflow == null) {
            return Common.badRequestWrapper("Workflow doesn't exist!");
        }

        String suggestionContent = jsonNode.path("sContent") == null? "NaN": jsonNode.path("sContent").asText();
        ...
        return created(new Gson().toJson(suggestions.getId()));  //post suggestion
    }

    public Result publishSuggestion() {
        Template temp = new SuggestionsController(workflowRepository,userRepository, suggestionsRepository);
        return temp.addItem();
    }
