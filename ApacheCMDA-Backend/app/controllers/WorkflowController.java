...
interface Criteria {
    public List<Workflow> meetCriteria(List<Workflow> workflows);
}


class IdInGroupCriteria implements Criteria {
    private List<Integer> groupsParse = new ArrayList<>();

    public IdInGroupCriteria(List<Integer> groupsParse) {
        this.groupsParse.addAll(groupsParse);
    }

    public List<Workflow> meetCriteria(List<Workflow> workflows) {
        List<Workflow> list = new ArrayList<Workflow> ();
        for(Workflow single: workflows) {
            if(groupsParse.contains((int)single.getGroupId())){
                list.add(single);
            }
        }
        return list;
...
class GroupIdEqZeroCriteria implements Criteria {

    public List<Workflow> meetCriteria(List<Workflow> workflows) {
        List<Workflow> list = new ArrayList<Workflow> ();
        for(Workflow single: workflows) {
            if(single.getGroupId() == 0){
                list.add(single);
            }
        }
        return list;
...
class NotDeletedCriteria implements Criteria {

    public List<Workflow> meetCriteria(List<Workflow> workflows) {
        List<Workflow> list = new ArrayList<Workflow> ();
        for(Workflow single: workflows) {
            if(!single.getStatus().equals("deleted")){
                list.add(single);
            }
        }
        return list;
...
class AndCriteria implements Criteria {
    ...
    public List<Workflow> meetCriteria(List<Workflow> workflows) {
        List<Workflow> c1Items = c1.meetCriteria(workflows);
        List<Workflow> c2Items = c2.meetCriteria(workflows);
        for (Workflow single : c2Items) {
            if(!c1Items.contains(single)){
                c1Items.add(single);
            }
        }
        return c1Items;
...
class OrCriteria implements Criteria {
    ...
    public List<Workflow> meetCriteria(List<Workflow> workflows) {
        List<Workflow> c1Items = c1.meetCriteria(workflows);
        List<Workflow> c2Items = c2.meetCriteria(c1Items);
        return c2Items;
    }
};
...
//        String wfOutput = json.path("wfOutput").asText();
        ...
        //builder pattern
        //http://www.javaworld.com/article/2074938/core-java/too-many-parameters-in-java-methods-part-3-builder-pattern.html?page=2
        Workflow workflow  = new Workflow.WorkflowBuilder().
                jsonNode(json).wfContributors(wfContributors).wfRelated(wfRelated)
                .userName(user.getUserName()).status("norm").user(user).createWorkflow();
    ...
    public Result getTimeLine(Long id, Long offset, String format) {
        ...
        if(followees.size()>0) {
            for (User followee: followees) {
                ...
                IdInGroupCriteria idInGroupCriteria = new IdInGroupCriteria(groupsParse);
                GroupIdEqZeroCriteria groupIdEqZeroCriteria = new GroupIdEqZeroCriteria();
                OrCriteria or = new OrCriteria(idInGroupCriteria, groupIdEqZeroCriteria);
                NotDeletedCriteria notDeletedCriteria = new NotDeletedCriteria();
                AndCriteria and = new AndCriteria(or,notDeletedCriteria);
                List<Workflow> WF = and.meetCriteria(workflows);
                allWorkflows.addAll(WF);
                for(Workflow single: WF) {
