...
// @LINE:72
def suggestGroupName(email:String, name:String): Call = {
   import ReverseRouteContext.empty
   Call("GET", _prefix + { _defaultPrefix } + "users/suggestGroupName/email/" + implicitly[PathBindable[String]].unbind("email", dynamicString(email)) + "/name/" + implicitly[PathBindable[String]].unbind("name", dynamicString(name)))
}
...
// @LINE:72
def suggestGroupName : JavascriptReverseRoute = JavascriptReverseRoute(
   "controllers.UserController.suggestGroupName",
   """
      function(email,name) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "users/suggestGroupName/email/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("email", encodeURIComponent(email)) + "/name/" + (""" + implicitly[PathBindable[String]].javascriptUnbind + """)("name", encodeURIComponent(name))})
      }
   """
)
...
// @LINE:72
def suggestGroupName(email:String, name:String): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.UserController]).suggestGroupName(email, name), HandlerDef(this.getClass.getClassLoader, "", "controllers.UserController", "suggestGroupName", Seq(classOf[String], classOf[String]), "GET", """""", _prefix + """users/suggestGroupName/email/$email<[^/]+>/name/$name<[^/]+>""")
)
